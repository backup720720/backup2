...
function EAL() {
    ...
    t.set_base_url();
    for (let i = 0; i < t.scripts_to_load.length; i++) {
    ...
    has_error: function () {
        ...
        editAreaLoader.lang["tr"]={
            new_document: "yeni boş döküman",
            search_button: "ara ve yerleştir",
            search_command: "ileri / arama alanını aç",
            search: "arama",
            replace: "yer değiştir",
            replace_command: "yer değiştir / arama alanını aç",
            find_next: "sonrakini bul",
            replace_all: "hepsini yer değiştir",
            reg_exp: "regexp",
            match_case: "benzerlik",
            not_found: "bulunamadı.",
            occurrence_replaced: "olay değiştirildi.",
            search_field_empty: "Arama kutusu boş",
            restart_search_at_begin: "Alanın sonuna ulaşıldı. Başlarken yeniden başlatın.",
            move_popup: "arama kutusunu taşı",
            font_size: "--Font size--",
            go_to_line: "satıra git",
            go_to_line_prompt: "satıra git:",
            undo: "geri al",
            redo: "ileri al",
            change_smooth_selection: "yüksek kalite",
            highlight: "sözdizimi vurgulamayı aç/kapat",
            reset_highlight: "vurgulamayı sıfırla",
            word_wrap: "kelime kaydırma modunu değiştir",
            help: "hakkında",
            save: "kaydet",
            load: "yükle",
            line_abbr: "Satır",
            ...
            position: "Pozisyon",
            total: "Toplam",
            close_popup: "pencereyi kapat",
            shortcuts: "Kısayollar",
            add_tab: "metne tablo ekleme",
            remove_tab: "metne tablo ekleme",
            about_notice: "N01",
            toggle: "Editörü aç/kapat",
        ...
        if (s["syntax"].length > 0) {
            ...
            t.load_script(t.baseURL + "src/reg_syntax/syntax.js");
        }
        eAs[s["id"]] = {"settings": s};
        ...
        let d = document, fs = window.frames, span, iframe;
        ...
        let t = this, d = document, f, span, father, next, html = '', html_toolbar_content = '', template, content, i;
        if (t.win != "loaded") {
        ...
        let fs = window.frames, f, t, parNod, nxtSib, selStart, selEnd, scrollTop, scrollLeft;
        if (fs["frame_" + id]) {
            f = fs["frame_" + id];
            t = eAs[id]["textarea"];
        ...
        let fs = window.frames, f, t, selStart = 0, selEnd = 0, scrollTop = 0, scrollLeft = 0, curPos, elem;
        if (fs["frame_" + id]) {
            f = fs["frame_" + id];
            t = eAs[id]["textarea"];
            area = f.editArea;
        ...
        let t = this, d = document, script, head;
        ...
        for (i in eAs) {
            ...
            if (window.frames["frame_" + i] && is_child && eAs[i]["displayed"] == true) {
                let exec = 'window.frames["frame_' + i + '"].editArea.textarea.value=document.getElementById("' + i + '").value;';
        ...
        let formObj, is_child, fs = window.frames, i, x;
        formObj = eAL.isIE ? window.event.srcElement : e.target;
        if (formObj.tagName != 'FORM') formObj = formObj.form;
        for (i in eAs) {
            is_child = false;
        ...
        let sel = this.getSelectionRange(id);
        return this.getValue(id).substring(sel["start"], sel["end"]);
    }, setSelectedText: function (id, new_val) {
        let fs = window.frames, d = document, sel, text, scrollTop, scrollLeft, new_sel_end;
        ...
        let fs = window.frames, d = document, t = this, scrollTop, scrollLeft, span;
        ...
        let fs = window.frames, d = document, t = this, span;
...
function get_css_property(elem, prop) {
    if (document.defaultView) {
        ...
        let prop = prop.replace(/-\D/gi, function (sMatch) {
...
function start_move_element(e, id, frame) {
    let elem_id = (e.target || e.srcElement).id;
    if (id) elem_id = id;
...
function get_IE_selection(t) {
    let d = document, div, range, stored_range, elem, scrollTop, relative_top, line_start, line_nb, range_start,
        range_end, tab;
    if (t && t.focused) {
        if (!t.ea_line_height) {
...
EAL.prototype.start_resize_area = function () {
    let d = document, a, div, width, height, father;
...
EAL.prototype.end_resize_area = function (e) {
    let d = document, div, a, width, height;
    d.onmouseup = "";
    d.onmousemove = "";
    div = d.getElementById("edit_area_resize");
    a = eAs[eAL.resize["id"]]["textarea"];
...
EAL.prototype.init_syntax_regexp = function () {
    let lang_style = {};
    for (let lang in this.load_syntax) {
        if (!this.syntax[lang]) {
            this.syntax[lang] = {};
            ...
            if (this.load_syntax[lang]['KEYWORDS']) {
                ...
                for (let i in this.load_syntax[lang]['KEYWORDS']) {
            ...
            if (this.load_syntax[lang]['OPERATORS']) {
                let str = "";
                let nb = 0;
                for (let i in this.load_syntax[lang]['OPERATORS']) {
            ...
            if (this.load_syntax[lang]['DELIMITERS']) {
                let str = "";
                let nb = 0;
                for (let i in this.load_syntax[lang]['DELIMITERS']) {
            ...
            if (this.load_syntax[lang]['QUOTEMARKS']) {
                for (let i in this.load_syntax[lang]['QUOTEMARKS']) {
                    if (typeof (this.load_syntax[lang]['QUOTEMARKS'][i]) == "function") continue;
                    let x = this.get_escaped_regexp(this.load_syntax[lang]['QUOTEMARKS'][i]);
            ...
            if (this.load_syntax[lang]['COMMENT_SINGLE']) {
                for (let i in this.load_syntax[lang]['COMMENT_SINGLE']) {
                    if (typeof (this.load_syntax[lang]['COMMENT_SINGLE'][i]) == "function") continue;
                    let x = this.get_escaped_regexp(this.load_syntax[lang]['COMMENT_SINGLE'][i]);
            ...
            if (this.load_syntax[lang]['COMMENT_MULTI']) {
                for (let i in this.load_syntax[lang]['COMMENT_MULTI']) {
                    if (typeof (this.load_syntax[lang]['COMMENT_MULTI'][i]) == "function") continue;
                    let start = this.get_escaped_regexp(i);
                    let end = this.get_escaped_regexp(this.load_syntax[lang]['COMMENT_MULTI'][i]);
            ...
            if (this.load_syntax[lang]['SCRIPT_DELIMITERS']) {
                ...
                for (let i in this.load_syntax[lang]['SCRIPT_DELIMITERS']) {
            ...
            if (this.load_syntax[lang]['REGEXPS']) {
                for (let i in this.load_syntax[lang]['REGEXPS']) {
                    if (typeof (this.load_syntax[lang]['REGEXPS'][i]) == "function") continue;
                    let val = this.load_syntax[lang]['REGEXPS'][i];
...
document.getElementById("codeArea").value = localStorage.getItem("code") || "";
editAreaLoader.init({
    id: "codeArea",
