...
class ImageModel extends _Model {
    ...
    setURL(url) {
        this.url = url;
        __loadImage(url, this.width, this.height).then(i => this.setImage(i));
        return this;
    }
    ...
    render(entity, scene) {
        ...
        if (!this.image && this.url) {
            ...
            const {url} = this;
            __loadImage(url, this.width, this.height).then(i => {
                this.setImage(i);
                draw();
            });
        } else draw();
...
class Scene {
    ...
    /**
     * @param {"onSetRunning", "onTick", "onTickEnd"} event
     * @param {function(event: Event)} callable
     * @returns {Scene}
     */
    on(event, callable) {
     ...
     */
    onTick(currentTick) {
        let ev = new Event("onTick", this);
        ev.currentTick = currentTick;
        ev.call();
        if(ev.isCancelled()) return this;
